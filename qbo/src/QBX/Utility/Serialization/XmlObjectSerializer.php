...
class XmlObjectSerializer extends IEntitySerializer
{
    ...
    private static function ParseArbitraryResultObjects($responseXml, $bLimitToOne)
    {
        ...
        foreach ($responseXmlObj as $oneXmlObj) {
            ...
            if ('Fault' == $oneXmlElementName) return NULL;
            ...
            if ($bLimitToOne) break;
        }

        if ($bLimitToOne) return $resultObject;
        else return $resultObjects;
    }
    ...
    {
        if ($idsLogger) $this->IDSLogger = $idsLogger;
        else $this->IDSLogger = NULL; // new TraceLogger(); 
    }
